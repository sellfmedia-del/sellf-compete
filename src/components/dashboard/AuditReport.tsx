  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-10 duration-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        {/* 1. KATEGORİ EN ÇOK SATANLAR */}
        <div className="md:col-span-8 bg-zinc-900/40 border border-zinc-800 rounded-[2.5rem] p-8 overflow-hidden relative group">
          <h3 className="text-lg font-black uppercase italic tracking-tight flex items-center gap-2 mb-6">
            ...
            {data.topSellers?.map((seller: any, i: number) => (
              <div key={i} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-orange-500/20 transition-all">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-zinc-800 rounded-xl flex items-center justify-center font-black text-orange-500 italic">#{i + 1}</div>
                  <span className="text-sm font-bold uppercase text-zinc-300">{seller.title}</span>
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-black text-zinc-500">Est. Sales</p>
                  <p className="text-sm font-black italic">{seller.sales_estimate}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="md:col-span-4 bg-zinc-900/40 border border-zinc-800 rounded-[2.5rem] p-8 flex flex-col justify-between">
          ...
          <div className="space-y-6">
            <div>
              <p className="text-[10px] font-black uppercase text-green-500 mb-2 flex items-center gap-1"><ThumbsUp size={12} /> Top Likes</p>
              <p className="text-xs text-zinc-400 leading-relaxed font-medium italic">"{data.sentiment?.likes}"</p>
            </div>
            <div className="h-px bg-zinc-800 w-full" />
            <div>
              <p className="text-[10px] font-black uppercase text-red-500 mb-2 flex items-center gap-1"><ThumbsDown size={12} /> Top Complaints</p>
              <p className="text-xs text-zinc-400 leading-relaxed font-medium italic">"{data.sentiment?.dislikes}"</p>
            </div>
          </div>
        </div>

        {/* 3. EN BÜYÜK VE YAKIN RAKİPLER */}
        <div className="md:col-span-4 bg-zinc-900/40 border border-zinc-800 rounded-[2.5rem] p-8">
          <h3 className="text-lg font-black uppercase italic tracking-tight mb-6 flex items-center gap-2"><Target size={20} className="text-orange-500" /> Strategic Rivals</h3>
          <div className="space-y-4">
             <div className="p-4 bg-orange-500 text-black rounded-2xl">
                <p className="text-[9px] font-black uppercase opacity-60">Main Threat</p>
                <h4 className="font-black text-lg leading-tight italic uppercase">{data.rivals?.mainThreat}</h4>
             </div>
             <div className="p-4 bg-white/5 border border-white/5 rounded-2xl">
                <p className="text-[9px] font-black uppercase text-zinc-500">Closest Match</p>
                <h4 className="font-black text-lg leading-tight italic uppercase">{data.rivals?.closestMatch}</h4>
             </div>
          </div>
        </div>
        ...
        <div className="md:col-span-8 bg-zinc-900/40 border border-zinc-800 rounded-[2.5rem] p-8 flex flex-col justify-between relative overflow-hidden group">
          <h3 className="text-lg font-black uppercase italic tracking-tight mb-4 flex items-center gap-2"><DollarSign size={20} className="text-orange-500" /> Price Positioning</h3>
          <div className="relative pt-12 pb-6">
            <div className="h-1 bg-zinc-800 w-full rounded-full relative">
               <div className="absolute left-[20%] right-[30%] h-full bg-orange-500/50" />
            ...
            <div className="flex justify-between mt-4 text-[10px] font-black uppercase tracking-widest text-zinc-600">
               <span>Min: ${data.pricing?.min}</span>
               <span className="text-orange-500">Optimal: {data.pricing?.optimalRange}</span>
               <span>Max: ${data.pricing?.max}</span>
            </div>
          </div>
        </div>

        {/* 5. PLATFORM BAZLI BİLGİLER */}
        <div className="md:col-span-6 bg-zinc-900/40 border border-zinc-800 rounded-[2.5rem] p-8 space-y-6">
          <h3 className="text-lg font-black uppercase italic tracking-tight flex items-center gap-2"><BarChart3 size={20} className="text-orange-500" /> Platform Dynamics</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
               <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1">Commission Rate</p>
               <p className="text-2xl font-black italic">{data.platformInfo?.commission}</p>
            </div>
            <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
               <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1">Supply/Demand</p>
               <p className="text-2xl font-black italic text-orange-500">{data.platformInfo?.demandLevel}</p>
            </div>
          </div>
          <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest leading-relaxed">Platform volume share: {data.platformInfo?.volumeShare}</p>
        </div>
        ...
        <div className="md:col-span-6 bg-zinc-100 text-black rounded-[2.5rem] p-8 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-6">
               ...
               <div className="bg-black text-white px-3 py-1 rounded-full text-[10px] font-black italic">{data.growth?.score} Score</div>
            </div>
            <div className="grid grid-cols-2 gap-4 mb-6">
               <div>
                  <h4 className="text-[10px] font-black uppercase mb-2 underline decoration-orange-500 decoration-2">Opportunity</h4>
                  <p className="text-[11px] font-bold leading-snug">{data.growth?.opportunity}</p>
               </div>
               <div>
                  <h4 className="text-[10px] font-black uppercase mb-2 underline decoration-orange-500 decoration-2">Threat</h4>
                  <p className="text-[11px] font-bold leading-snug">{data.growth?.threat}</p>
               </div>
            </div>
          </div>
          <div className="pt-6 border-t border-black/10 flex items-center justify-between">
             <span className="text-xs font-black uppercase italic opacity-60 tracking-tighter">Est. Revenue Forecast</span>
             <span className="text-3xl font-black italic tracking-tighter">{data.growth?.revenueForecast}</span>
          </div>
        </div>
      </div>
