'use client';

import React, { useState } from 'react';
import AuditTypeSelector from '@/src/components/dashboard/AuditTypeSelector';
import AuditInputForm from '@/src/components/dashboard/AuditInputForm';
import AuditReport from '@/src/components/dashboard/AuditReport'; // Bir sonraki adımda detaylandıracağız

export default function DashboardPage() {
  const [step, setStep] = useState(1); // 1: Choice, 2: Input, 3: Result
  const [auditType, setAuditType] = useState<'product' | 'idea' | null>(null);

  const handleTypeSelect = (type: 'product' | 'idea') => {
    setAuditType(type);
    setStep(2);
  };

  const startAudit = () => {
    setStep(3);
    // Burada ileride API çağrısı yapılacak
  };

  return (
    <div className="p-8 max-w-5xl mx-auto space-y-12 pb-24">
      
      {/* ADIM 1: SEÇİM EKRANI */}
      {step === 1 && (
        <div className="pt-12 animate-in fade-in zoom-in-95 duration-500">
           <div className="text-center mb-12">
              <h2 className="text-3xl font-black tracking-tighter uppercase italic text-white">Start New Audit</h2>
              <p className="text-zinc-500 text-sm mt-2 font-medium uppercase tracking-widest">Select your entry point to the ecosystem</p>
           </div>
           <AuditTypeSelector onSelect={handleTypeSelect} />
        </div>
      )}

      {/* ADIM 2: GİRDİ EKRANI */}
      {step >= 2 && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
          <AuditInputForm 
            type={auditType!} 
            onBack={() => setStep(1)} 
            onStart={startAudit}
          />
        </div>
      )}

      {/* ADIM 3: RAPOR EKRANI */}
      {step === 3 && (
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000">
          <div className="h-px bg-zinc-900 w-full my-16" />
          <AuditReport />
        </div>
      )}

    </div>
  );
}